...
def performance_stats(policies, rewards, matches):
    ...
    policy_set = [''.join(p) for p in policy_set]
    unique_policy = set([''.join(p) for p in policy_set])
...
def load_weights_to_flatresnet(source_model, target_model):
    ...
    if all(source_keys.startswith('module.') for source_keys in list(source_state.keys())):
...
def get_dataset(model, root=os.getcwd()+'/data'):
    rnet, dset = model.split('_')[:2]
    ...
    if dset=='C10':
        ...
        trainset = torchdata.ImageFolder('~/research/dataset/ILSVRC/Data/CLS-LOC/train', transform_train)
        testset = torchdata.ImageFolder('~/research/dataset/ILSVRC/Data/CLS-LOC/train', transform_test)
...
def get_model(model):
    ...
    elif model=='R110_C10_TR':
        rnet_checkpoint = 'cv/pretrained/R110_C10/pk_E_130_A_0.932.t7'
        layer_config = [18, 18, 18]
        base_rnet = resnet.FlatResNet32(base.BasicBlock, layer_config, num_classes=10)
        #rnet_1 = 
        agent = resnet.Policy32([1,1,1], num_blocks=27)
...
def layer_selcet(model, reward):
    rnet, _ = get_model(model)
    num_layers = len(list(rnet.children()))
    new_reward = [reward - (reward/num_layers)*current_layer for current_layer in num_layers]
