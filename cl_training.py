...
def get_reward(preds, targets, policy):
    ...
    for batch_idx, current_policy in enumerate(policy):
        for idx, value in enumerate(current_policy):
            if args.model == "R110_C10_TR":
                policy[batch_idx][idx] = value
            else:
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in tqdm.tqdm(enumerate(trainloader), total=len(trainloader)):
        ...
        v_inputs = Variable(inputs.data, requires_grad=True)
        ...
        policy = policy[:,:agent.num_blocks]
        loss = -distr.log_prob(policy)
        loss = loss * Variable(advantage).expand_as(policy).cuda()

        if policy_mask is not None:
            policy_mask = policy_mask[:,:agent.num_blocks]
...
def test(epoch):
    ...
    for batch_idx, (inputs, targets) in tqdm.tqdm(enumerate(testloader), total=len(testloader)):
        ...
        inputs, targets = Variable(inputs, requires_grad=True), Variable(targets).cuda(non_blocking=True)
        if not args.parallel:
            inputs = inputs.cuda()
